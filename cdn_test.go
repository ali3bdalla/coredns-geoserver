...
func TestCDN(t *testing.T) {
	...
	ctx := context.TODO()
	r := new(dns.Msg)
	r.SetQuestion("cdn.org.", dns.TypeA)
	...
	if a := b.String(); !strings.Contains(a, "[INFO] plugin/cdn: cdn") {
		t.Errorf("Failed to print '%s', got %s", "[INFO] plugin/cdn: cdn", a)
	}
}
